...
class _StudentLeavesState extends State<StudentLeaves> {
  ...
  void getLeaveList() async {
    ...
    final response = await http
      ...
      print(loader);
      print('Lenght:${listofLeaves.length}');
    }).catchError((onError) {
      print(onError);
    });
  }
  ...
  Future<String> apiRequest(String url, Map dataMap) async {
    ...
    Map<String, dynamic> leaveDone = jsonDecode(reply);
    print('Leave Done:$leaveDone');
    var done = leaveDone['done'];
    _applyLeaveDone(done);
    setState(() {
      visible = false;
    });
  ...
  void _applyLeaveDone(String leave) {
    if (leave == 'done') {
      Toast.show('Leave Apply Successful', context,
        ...
        TableRow(
          children: [
            ...
            IconButton(
                icon: files == ""
                    ? Text('')
                    : Icon(Icons.link, color: Color.fromRGBO(33, 23, 47, 1)),
                onPressed: () {
                  print(files);
                  return showDialog(
                    context: context,
            ...
            : SingleChildScrollView(
                child: Column(
                  children: <Widget>[
                    ...
                    visible ? bodyProgress : Container(),
